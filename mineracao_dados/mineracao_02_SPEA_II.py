...
class AG():
    ...
    def calcular_shared_distance(self, posicao, pop_fron,menor_fitness1,maior_fitness1,menor_fitness2,maior_fitness2):
        ...
        distancia = abs(f1_dis) + abs(f2_dis)
        return distancia
    ...
    def ordena_populacao_fronteira(self):
        self.populacao = sorted(self.populacao, key=lambda populacao: populacao.fronteira)
    ...
    def torneio_estocastico(self):
        ...
        while cont < 3:
            ...
            if(individuo.fronteira < fronteira):
                maior = copy.deepcopy(self.populacao[p].fitness)
                posicaoRetorno = p
            elif (individuo.fronteira == fronteira):
                if(individuo.shared_fitness >= shared):
                    maior = copy.deepcopy(self.populacao[p].fitness)
                    posicaoRetorno = p
            cont += 1
    ...
    def geracoes(self):
        ...
        filhosR = []
        filhosR.clear()
        print("Fitness 1  ", melhor.fitness," F-score ",melhor.fitness2," simplicidade da Regra",melhor.fitness3)
...
def main():
    ...
    while ngeracao < geracao:
        ...
        for i in a.populacao:
            print("Fronteira ", i.fronteira, " Fitness 1 ", i.fitness, "  F-score ", i.fitness2,"  Simplicidade da regra ", i.fitness3, " Shared ",i.shared_fitness)
        a.ordena_populacao_fronteira()
