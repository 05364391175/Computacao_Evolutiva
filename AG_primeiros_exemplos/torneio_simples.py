...
class AG():
    ...
    def torneio_simples(self,tuor):
        ...
        maior = 0 #Captura o maior fitness do torneio
        cromossomoRetorno = []
        while tuor > 0:
            selecionado = random.randint(0, len(self.populacao)-1)
            selecionados.append(copy.deepcopy(self.populacao[selecionado]))
            if(maior < self.populacao[selecionado].fitness):
                maior = copy.deepcopy(self.populacao[selecionado].fitness)
                cromossomoRetorno = copy.deepcopy(self.populacao[selecionado].cromossomo)
            tuor -=1
        return cromossomoRetorno
    ...
    def criaGeracao(self,totalPares):
        self.ordena_populacao_maior()
        listaFilhosGerados = []
        listaFilhosGerados.clear()
        for i in range(0,int(totalPares/2)):
            pai1 = AG.torneio_simples(self,3)
            pai2 =  AG.torneio_simples(self,3)
            #print("PAI 1", pai1)
            #print("PAI 2", pai2)
        ...
        #Para tirar da reiserção ordenada
        """
        self.ordena_populacao_maior
